...
export const ui = {
  ...
  calendarInit(ce, viewMonth, datesSet) {
    ...
    const handleEventMouseEnter = (info) => {
      ...
      $(info.el).css("margin", "0").find(".fc-event-title").css("font-size", "20px");
    ...
    const handleEventMouseLeave = (info) => {
      ...
      $(info.el).css("margin", "1px 2px 0 2px").find(".fc-event-title").css("font-size", "16px");
    ...
    let calendar = new Calendar(calendarElm, {
      ...
      initialView: "dayGridMonth",
      initialDate: new Date(`2024-${viewMonth < 10 ? "0" + viewMonth : viewMonth}-01`),
  ...
  jexcelFocus(i, j) {
    setTimeout(() => {
      ...
      $(".content .content_wrap").scrollTop($(`.jexcel tbody tr:eq(${i}) td:eq(${j + 1})`).position().top - $(".content .content_wrap").height() / 2);
    }, 200);
  },
};
